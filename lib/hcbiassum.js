...
//add tooltip to svg
  ...
  var blue_g = g.append("g").attr("id","blue_g");
  var red_g = g.append("g").attr("id","red_g");
  ...
  religion_bias.forEach(function(d){
      console.log(d.Bias_Motivation_Category + " : " + d.Year + " : " + d.Incidents);
  });
  ...
  var annotations = [{
    ...
    x: 290 + margin.left, y: 67 + margin.bottom * 0.25,
    dy: -30,
    dx: 0,
    ...
    x: 580 + margin.left, y: 225 + margin.bottom * 0.25,
    dy: -30,
    dx: 0,
    ...
    x: 798 + margin.left, y: 242 + margin.bottom * 0.25,
    dy: -30,
    dx: 0,
    subject: { text: 'B', y:"bottom" },
    id: "minimize-badge"
    ...
    y.domain([0, d3.max(data, function(d) { return d.Incidents; })+ 500]);
    ...
    red_g.append("path")
     ...
     red_g.selectAll(".dot")
        .data(religion_bias.filter(function(d) { 
            console.log("Bias Motivation: " + d.Bias_Motivation_Category + " Year:" + d.Year +  " x coordinate: " + x(d.Year) +" Incidents: " + d.Incidents + " y coordinate: " + y(d.Incidents));
            return d; }))
        .enter().append("circle")
        .attr("class", "dot")
